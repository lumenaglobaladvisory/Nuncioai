...
Automated reminder/compliance systems (government portals, vendor certification trackers, etc.) often send each follow-up as a brand-new thread rather than a reply, so a request the user already fulfilled can still look "unanswered" in isolation. Treat hedging language like "if you've already submitted this, please disregard" or "if this no longer applies to you, disregard this notice" as a signal the deadline is not reliable - never classify a thread containing that kind of disclaimer as must_respond_today; review_this_week at most.
